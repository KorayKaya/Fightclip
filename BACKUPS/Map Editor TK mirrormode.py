...
def createBlockP(event):
    global startx,starty,f,canvas,create,killer,mirror,k
    create=False
    if len(canvas.find_overlapping(event.x, event.y, event.x, event.y))==0:
        if not killer:
            ...
            if mirror:
                k=canvas.create_rectangle(700-event.x,event.y,700-event.x+1,event.y+1, fill="white",outline="white",tags="normal")
        elif killer:
            f=canvas.create_rectangle(event.x,event.y,event.x+1,event.y+1, fill="grey",outline="red",tags="killer")
            if mirror:
                k=canvas.create_rectangle(700-event.x,event.y,700-event.x+1,event.y+1, fill="grey",outline="red",tags="killer")
        print(f)
        startx=event.x
        starty=event.y
...
def createBlockM(event):
    ...
    if create:
        ...
        if mirror:
            canvas.coords(k, (700-startx,starty,700-event.x,event.y))
def createBlockR(event):
    global f,rectlist,listb,create,killer,k,mirror
    if create:
        if mirror:
